 ...
 *
 *******************************************************************************************************/
#include "drivers/TC123X/driver.h"
#include "ext_misc.h"
...
{
	if( (unsigned int)( sys_timer_tick  - (reg_system_tick + 7*CLOCK_SYS_TIMER_CLK_1US )) > BIT(30) )//63us - 75us in flash / 3.5us in ram
	{
		reg_system_tick_irq = reg_system_tick + 12*CLOCK_SYS_TIMER_CLK_1US; ///320 = 0x140, so not &0xfffffff8
	}
	else{
